...
Cohesion: 0.23
Nodes (11): arith_rshift(), conjugate_multiply_sample(), decimate(), process_block(), Quick sanity checks for each module., Convert signed Python int to unsigned representation in `bits` width., Saturate a value to signed `bits`-wide range [-2^(bits-1), 2^(bits-1)-1]., Arithmetic right shift. Python >> on signed int is already arithmetic. (+3 more)
