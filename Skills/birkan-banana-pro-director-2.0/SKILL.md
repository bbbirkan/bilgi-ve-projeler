...
Before any prompt, ask the user which tool to use for the face lock. Four options:

> Want to build this in Banana Pro, Nano Banana 2, GPT-2, or Soul Cinema?
> — **Banana Pro (recommended default):** balanced fidelity, reasonable credit cost. Works for most character builds straight up. Single-pass build, same prompt structure as Nano Banana 2.
> — **Nano Banana 2 (fast + cheap iteration):** built on Gemini 3.1 Flash — roughly 2–3× faster and ~half the credit cost of Banana Pro, near-Pro quality on standard character builds. Best when you want to iterate quickly across multiple face variations before committing to a final lock. Uses the same prompt structure as Step 0.A. Has Thinking Mode and Image Search Grounding for better real-world subject accuracy.
...
> — **Soul Cinema (loosest, fastest exploration):** good when the user isn't sure yet and wants rough variations on the face register. Lowest fidelity of the four. If used, run as Step 0.1 first to produce a face plate, then a Banana Pro 3:4 pass (Step 0.2) to lock the finer detail.
