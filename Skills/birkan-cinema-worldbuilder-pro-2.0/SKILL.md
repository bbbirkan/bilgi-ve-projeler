...
24. **Never stack "fast" across camera and subject simultaneously.** "Fast" is the single keyword most likely to degrade output quality — combining fast camera movement + fast cuts + busy subject motion almost guarantees jitter and artifacts. If speed is needed in the shot, choose only one element to be fast and keep everything else slow or held.
...
**Camera-subject separation rule (Seedance 2.0).** Camera movement and subject movement must live in separate, complete sentences. Never write "the camera spins around her as she dances" — write "She dances in place, weight shifting with the beat. Camera holds locked framing." Mixing both into one clause is the most common Seedance parsing failure and produces uncontrolled, shaky output. State subject action first, camera action second, always in distinct sentences.
